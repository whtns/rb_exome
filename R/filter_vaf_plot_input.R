...
filter_vaf_plot_input <- function(vaf_plot_input) {
    ...
    # dropped_snps <- dplyr::anti_join(vaf_plot_input, filtered_vaf_plot_input) %>% 
    #   ungroup() %>% 
    #   dplyr::select(sample_id, snp_id) %>% 
    #   dplyr::mutate(sanger_panel = "*")
    # 
    # sanger_panels <- 
    #   dplyr::bind_rows(sanger_panels, dropped_snps) %>% 
    #   write_csv("sanger_panels_new.csv")
    ...
    niggling_snps <- c("PABPN1L_24-T_A", "ZNF41_24-T_C", "SLC7A3_24-T_T")
