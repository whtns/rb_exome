...
plot_all_study_qc <- function(all_study_qc) {
    ...
    my_comparisons <- list(c("McEvoy", "Stachelek T"), c("Kooi", "Stachelek T"), c("Zhang", "Stachelek T"), c("Stachelek CL", "Stachelek T"))
    
    vaf_per_study_box_plot <- 
        all_study_qc$vaf_per_study %>% 
        dplyr::mutate(sample_set = factor(sample_set, levels = c("Zhang", "Kooi", "McEvoy", "Stachelek T", "Stachelek CL"))) %>% 
