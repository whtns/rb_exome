...
glue("In addition to recurrent SCNAs, whole genome or exome sequencing of {sum(n_ngs_samples$n)} treatment-naïve retinoblastomas revealed pathogenic mutation or focal deletion of BCOR in {n_BCOR}, CREBBP variants in {english(n_CREBBP)}, and potentially pathogenic variants of {nrow(private_prior_ngs_snvs)} genes thus far reported for only one sample")
...
```{r}
glue("Past whole exome or whole genome sequencing of {sum(n_ngs_samples$n)} treatment-naïve retinoblastomas revealed recurrent secondary mutations or focal deletion in BCOR in ({n_BCOR}) and CREBBP (in {n_CREBBP}) (Supplementary Table S9), and {nrow(private_prior_ngs_snvs)} potentially pathogenic private somatic mutations ")
...
glue("Filtering such variants revealed a total of {sum(stachelek_filtered_tumor_variants)} somatic mutations in 12 tumors (mean {round(mean(stachelek_filtered_tumor_variants), 2)}; median {round(median(stachelek_filtered_tumor_variants), 2)}), of which {english(nrow(filtered_circle_ids$positive))} were subclonal variants detected based on their enrichment and detection in a matched cell line")
...
glue("Among the {sum(stachelek_filtered_tumor_variants)} somatic variants detected in the 12 tumors, five were in four genes in which mutations were detected in prior retinoblastoma exome or genome sequencing, including BCOR in two samples and PAN2, NAF1, and SAMD9 in one sample each (Table S1, Fig 4C).")

glue("Each was present at subclonal levels in tumors and their VAFs either increased or decreased during cell line establishment (Fig. 4). Specifically, in CHLA-VC-RB29, the PAN2 G794A VAF increased from {PAN2_vaf[[2]]} to {PAN2_vaf[[1]]} indicative of clonal selection (Fig. 4). In CHLA-VC-RB43, the SAMD9 P635S VAF increased from {SAMD9_vaf[[2]]} to {SAMD9_vaf[[1]]}, remaining subclonal but enriched. In contrast, CHLA-VC-RB24 the NAF1 R255Q VAF decreased from {NAF1_vaf} in T24 to undetected in CL24, and in CHLA-VC-RB28 the BCOR R1400IfsX12 VAF declined from {BCOR28_vaf} in T28 to undetected in CL28 (Fig. 4A), consistent with the cell lines’ derivation from subclones that lacked SCNAs that were prominent in the respective tumors (Fig. 2A). Finally, in CHLA-VC-RB33, BCOR G756X VAF declined from {BCOR33_vaf[[2]]} to {BCOR33_vaf[[1]]}, indicating partial selection against the BCOR G756X-containing cells (Fig. 4A). The decreased NAF1 and BCOR VAFs in tumor-derived cell lines implied a lack of selection for these variants during cell line establishment.")

glue("Among the {nrow(stachelek_filtered_cell_line_only_variants)} potentially pathogenic somatic variants detected only in cell lines, only one (ATM) was in a gene  in which variants had been detected in prior retinoblastoma sequencing. When combined with re-analyses of past whole exome or genome sequencing, our detection of PAN2, NAF1, and SAMD9, recognition of recurrent alterations in CLIP2, and prior recognition of recurrent changes in HRG, MGA, and ARID1A, gives a total of {length(all_recurrent)} recurrently altered genes (Fig. 4C).") 
     
glue("The {english(n_distinct(recurrent_ngs_genes))} recurrently altered genes identified by whole exome or whole genome sequencing is significantly more than the n# of recurrences expected by chance among {nrow(all_ngs_snvs)} variants identified in WGS/WES studies.")
...
glue("Gene variants detected in 15 CHLA-RB cell lines included potentially pathogenic mutations in {n_distinct(reynolds_mutations$Gene)} genes (mean {round(n_distinct(reynolds_snv$Gene)/15)} per sample), a higher number than for the CHLA-VC-RB cell lines likely reflecting the lack of filtering of private germline polymorphisms. The mutations affected BCOR, CREBBP and {n_distinct(recurrent_reynolds_mutations$Gene)} other genes that were mutated in other sequenced retinoblastoma samples and are candidate recurrent tumor variants (Supplementary Table S1009)")
```
...
```{r}
glue("Similarly, this approach revealed {sum(stachelek_filtered_tumor_variants)} mutations in the 12 cell lines (mean: {round(mean(stachelek_filtered_tumor_variants), 2)}; median: {round(median(stachelek_filtered_tumor_variants))}), of which one was identified based on its initial detection in the originating tumor  (Fig. 4A, B, Table S4) and of which 17 were detected in cell lines as well as matched tumors.")
...
These analyses revealed enrichment for ontology terms with FDR <0.05 related to mitotic sister chromatid segregation, ribonucleoprotein (RNP) complex assembly, several RNP assembly related terms, and covalent chromatin modification")
